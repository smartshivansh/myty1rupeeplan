...
router.post("/add-with-query", async (req, res) => {
  ...
  console.log({
    query,
    type: action ? { $pull: { feeds: "data" } } : { $push: { feeds: "data" } },
  });

  switch (query.type) {
    case "BLOG":
      const blog = await BLOG(query.data, action);
      res.status(200).json({ msg: "success", data: blog });
      // data && res.status(200).send({ msg: "success", data: data[0].feeds[0] });
      break;

    case "IMAGE":
      const image = await IMAGE(query.data, action);
      res.status(200).json({ msg: "success", data: image });
      // res.status(200).send({ msg: "success", data: image.feeds.length });
      break;

    case "VIDEO":
      const video = await VIDEO(query.data, action);
